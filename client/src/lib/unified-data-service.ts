...
class UnifiedDataServiceImpl implements UnifiedDataService {
  ...
  async acceptOffer(offerId: string): Promise<any> {
    ...
    if (this.isProduction()) {
      console.log('🔥 Production: Using direct Firestore for offer acceptance');
      await this.initFirebase();
      const { updateDoc, doc, serverTimestamp, getDoc } = await import('firebase/firestore');
      
      try {
        // First verify the offer exists and user has permission
        ...
        console.log('📋 Offer data:', { toUserId: offerData.toUserId, currentUserId: currentUser.id });
      ...
      console.log('🛠️ Development: Using API for offer acceptance');
      try {
        const response = await apiRequest('PATCH', `/api/offers/${offerId}`, { status: 'accepted' });
        return await response.json();
      } catch (error) {
        console.error('❌ API offer acceptance error:', error);
        // Fallback to direct Firestore
        console.log('🔄 API failed, falling back to direct Firestore');
        await this.initFirebase();
        const { updateDoc, doc, serverTimestamp, getDoc } = await import('firebase/firestore');
        ...
        try {
          ...
          console.log('✅ Fallback: Offer accepted successfully');
          return { success: true };
        } catch (fallbackError) {
  ...
  async rejectOffer(offerId: string): Promise<any> {
    ...
    if (this.isProduction()) {
      console.log('🔥 Production: Using direct Firestore for offer rejection');
      await this.initFirebase();
      const { updateDoc, doc, serverTimestamp, getDoc } = await import('firebase/firestore');
      ...
      console.log('🛠️ Development: Using API for offer rejection');
      try {
        const response = await apiRequest('PATCH', `/api/offers/${offerId}`, { status: 'declined' });
        return await response.json();
      } catch (error) {
        console.error('❌ API offer rejection error:', error);
        // Fallback to direct Firestore
        console.log('🔄 API failed, falling back to direct Firestore');
        await this.initFirebase();
        const { updateDoc, doc, serverTimestamp, getDoc } = await import('firebase/firestore');
        ...
        try {
          ...
          console.log('✅ Fallback: Offer rejected successfully');
          return { success: true };
        } catch (fallbackError) {
          console.error('❌ Fallback error:', fallbackError);
          throw fallbackError;
